...
4. Ruan historikun e të dhënave dhe analizave në **MongoDB**, në mënyrë që platforma të mund të krahasojë performancën muaj pas muaji dhe të projektojë rritjen e mundshme të fitimit nëse klienti zbaton rekomandimet.
...
| 5 | Integrim me MongoDB për ruajtje historike | Databazë e strukturuar për krahasime periodike |
| 6 | Simulim/parashikim i ndikimit të rekomandimeve në fitim | Modul "what-if" brenda aplikacionit |
...
- Aplikacion Streamlit me:
  ...
  - Krahasim muaj-me-muaj dhe projeksion i rritjes së fitimit nëse zbatohen rekomandimet.
- Integrim me MongoDB (ruajtja e çdo upload-i, metadata, rezultatet e analizave).
...
- **pymongo / MongoDB Atlas (ose lokal)** – ruajtja e të dhënave dhe historikut
...
| **Faza 6 – Integrimi me MongoDB** | Skema e koleksioneve, ruajtja e historikut, krahasime muaj-me-muaj | 4-5 ditë |
| **Faza 7 – Moduli "What-if" / Projeksion Fitimi** | Simulim i ndikimit financiar nëse zbatohen rekomandimet | 4-5 ditë |
| **Faza 8 – Testim & Rregullim** | Testim me dataset real dhe me CSV muaj-provë, rregullim gabimesh | 3-4 ditë |
| **Faza 9 – Dorëzimi & Trajnimi i Klientit** | Dokumentacion, video/udhëzues përdorimi, sesion trajnimi | 2 ditë |
...
- Grafik krahasues muaj-me-muaj (pas integrimit me MongoDB).
...
5. **Krahasimi Historik**
   - Grafikë që krahasojnë muajin aktual me muajt e mëparshëm (nga MongoDB).
6. **Simulimi "What-if"**
...
## 9. Integrimi me MongoDB — Struktura e Propozuar

| Koleksioni | Përmbajtja |
|------------|------------|
| `raw_uploads` | Të dhënat e papërpunuara të çdo CSV të ngarkuar, me timestamp dhe metadata |
| `monthly_analytics` | Rezultatet e përpunuara (KPI, agregime) për çdo muaj |
| `recommendations_log` | Rekomandimet e gjeneruara për çdo periudhë dhe statusi i tyre (zbatuar/jo) |
| `kpi_history` | Seri kohore e KPI-ve kryesore për krahasime dhe grafikë trendi |

Kjo strukturë mundëson që platforma të "mësojë" nga çdo muaj i ri dhe të bëjë krahasime afatgjata pa ri-analizuar gjithçka nga fillimi.

---

## 10. Kriteret e Suksesit (Deliverables Finale)
...
| Volum i madh të dhënash (performancë) | Ngadalësim i aplikacionit | Përdorim i indeksimit në MongoDB dhe caching në Streamlit |
| Siguria e të dhënave të klientit | Konfidencialitet | Autentikim bazik + akses i kufizuar në databazë |
...
2. Marrja e qasjes/kredencialeve për MongoDB (nëse klienti ka preferencë specifike: lokal, Atlas, etj.).
